...
window.onload = function() {
    ...
    let price = document.getElementsByTagName("span")[0];
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        arr[i].addEventListener('dragstart', function(evt) {
            ...
            // Метод setData(...) сообщает механизму перетаскивания в браузере, какие данные из перетаскиваемого объекта
            //должен «поймать» целевой элемент, также называемый зоной приема. Здесь мы указываем, что передаваемые
            //данные это id элемента
    ...
    target.addEventListener("dragover", function(evt) {
        ...
        // отменяем стандартное обработчик события dragover. реализация данного обработчика по умолчанию не позволяет 
        // перетаскивать данные на целевой элемент, так как большая часть веб страницы не может принимать данные.
